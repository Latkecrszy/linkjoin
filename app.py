...
def update():
    ...
    if request.cookies.get('email'):
        ...
        if data.get('repeats')[0].isdigit():
            insert['occurrences'] = (int(data.get('repeats')[0])) * len(data.get('days'))
        """
        shared_links = mongo.db.links.find({'shared_id': int(data.get('id'))})
        for link in shared_links:
            mongo.db.links.find_one_and_update({'username': link['username'], 'id': link['id']}, {'$set': {
                'name': link['name'],
                'time': link['time'],
                'days': link['days'],
                'link': encoder.encrypt((encoder.decrypt(link['link']))),
                'repeat': link['repeat'],
...
<html>
    ...
    '''.format(token=token), subtype='html')
